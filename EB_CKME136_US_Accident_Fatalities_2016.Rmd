...
#' Create fixed sampling scheme (10-folds) so as to compare the different models
...
evaluator <- function(subset) {
  m <- train(as.simple.formula(subset, "type"), data = zooo, method = "rpart",
...
```{r}
fit <- train(FATALS ~ ., data=zoom, method = "rpart",
...
```{r}
confusionMatrix(data = predict(fit, zoom),
...
```{r}
library(sampling)
id <- strata(zoom, stratanames="FATALS", size=c(22731,11708), method="srswr")
zoom_balanced <- zoom[id$ID_unit, ]
table(zoom_balanced$FATALS)
```

```{r}
fit <- train(FATALS ~ ., data = zoom_balanced, method = "rpart",
  trControl = trainControl(method = "cv"),
...
confusionMatrix(data = predict(fit, zoom_balanced),
  ref = zoom_balanced$type, positive = "single")
```
...
```{r}
confusionMatrix(data = predict(fit, zoom),
  ref = Zoo_reptile$type, positive = "single")
```
